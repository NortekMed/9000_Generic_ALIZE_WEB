<%@ Page Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="OBS_Turbi.aspx.cs" Inherits="Turbi_obs"%>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
    ...
    <asp:HiddenField ID = "page_name"  value="<%$ Resources:Site.master, turbi %>" Runat="Server" />
    ...
    <asp:HiddenField ID = "Alim_name"  ClientIdMode="Static" Runat="Server" />
    <asp:HiddenField ID = "Alim_unit"  ClientIdMode="Static" Runat="Server" />
    <asp:HiddenField ID = "Alime_label" ClientIdMode="Static" Runat="Server" />

    <asp:HiddenField ID = "NTU_1_name"  ClientIdMode="Static" Runat="Server" />
    <asp:HiddenField ID = "NTU_1_unit"  ClientIdMode="Static" Runat="Server" />
    <asp:HiddenField ID = "NTU_1_label" ClientIdMode="Static" Runat="Server" />

    <asp:HiddenField ID = "NTU_2_name"  ClientIdMode="Static" Runat="Server" />
    <asp:HiddenField ID = "NTU_2_unit"  ClientIdMode="Static" Runat="Server" />
    <asp:HiddenField ID = "NTU_2_label" ClientIdMode="Static" Runat="Server" />

    <asp:HiddenField ID = "Bat_name"  ClientIdMode="Static" Runat="Server" />
    <asp:HiddenField ID = "Bat_unit"  ClientIdMode="Static" Runat="Server" />
    <asp:HiddenField ID = "Bat_label" ClientIdMode="Static" Runat="Server" />
    ...
    <%--<script type="text/javascript">
        var label = document.getElementById('<%=Alim_label.ClientID%>').value;
    ...
    <script type="text/javascript">
        var label = document.getElementById('<%=NTU_1_label.ClientID%>').value;

        document.write('<div class="panel panel-default">');
        document.write('<div class="panel-heading"><b>' + label + '</b></div>');
        document.write('<div class="panel-body">');
        document.write('<div id="NTU_1_container" style="min-width:500px; width:100%; height:300px;"></div>');
        document.write('</div>');
        document.write('</div>');
    </script>

    <%--<script type="text/javascript">
        var label = document.getElementById('<%=NTU_2_label.ClientID%>').value;
    ...
    <%--<script type="text/javascript">
        var label = document.getElementById('<%=Bat_label.ClientID%>').value;
    ...
    <script type="text/javascript">
        ...
        function updateCharts(data) {
            ...
            // Param0 chart -- Temp
            var chartpar0 = $('#NTU_1_container').highcharts();
            ...
            for (var i = 0; i < data.str_time.length; i++) {
                par0.push([Date.parse(data.str_time[i].replace(/\-/g, '\/').replace(/T/, ' ').replace(/Z/, ' -0')), data.ntu_1[i]]);
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var par0_label = document.getElementById('<%=NTU_1_label.ClientID%>').value;
            var par0_name = document.getElementById('<%=NTU_1_name.ClientID%>').value;
            var par0_unit = " " + document.getElementById('<%=NTU_1_unit.ClientID%>').value;
